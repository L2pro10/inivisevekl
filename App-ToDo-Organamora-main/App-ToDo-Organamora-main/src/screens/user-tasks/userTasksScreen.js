...
import { deleteUserTask, getUserTask } from '../../services/userTasksServices.js';
...
export function UserTaskScreen({ navigation }) {
  ...
  const searchUsers = (text) => {
    ...
    if (text.trim() === '') {
        setFilteredUsersTasks(usersTasks);
    } else {
  ...
  const renderTask = (task, userId) => (
    <View key={task.id_tasks} style={styles.taskCard}>
      ...
      <TouchableOpacity style={generalStyles.button} onPress={() => deleteUserTask(userId, task.id_tasks)}>
    ...
    <View style={styles.userCard}>
      ...
      {expandedUser === item.id_users && (
        <View style={styles.tasksContainer}>
          ...
          {item.tasks?.length > 0 ? (
            item.tasks.map(task => renderTask(task, item.id_users))
          ) : (
